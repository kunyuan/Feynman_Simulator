...
#include "module/weight/weight_class.h"
...
WeightBase::WeightBase(const Lattice &lat, real beta, int SpinVol, const string &name)
{
    _Beta = beta;
    _dBeta = beta / MAX_BIN;
    ...
    // _Shape[ORDER] does not initialized here
    // because ORDER may not be needed by the inherited class
    _Shape[SP] = SpinVol;
...
int WeightBase::SpinIndex(spin SpinIn, spin SpinOut)
{
    return SpinIn * SPIN + SpinOut;
}

int WeightBase::SpinIndex(spin *TwoSpinIn, spin *TwoSpinOut)
{
    return TwoSpinIn[0] * SPIN3 + TwoSpinIn[1] * SPIN2 +
           TwoSpinOut[0] * SPIN + TwoSpinOut[1];
}

int WeightBase::TauToBin(real t_in, real t_out)
{
    return TauToBin(t_out - t_in);
}

int WeightBase::TauToBin(real tau)
{
    if (DEBUGMODE && tau < -_Beta || tau >= _Beta)
        LOG_INFO("tau=" << tau << " out of the range ["
                        << -_Beta << "," << _Beta << ")");
    //TODO: mapping between tau and bin
    return tau < 0 ? int(tau * _dBetaInverse) + MAX_BIN
                   : int(tau * _dBetaInverse);
}

real WeightBase::BinToTau(int Bin)
{
    //TODO: mapping between tau and bin
    return Bin * _dBeta + _Beta / 2;
}

WeightArray::WeightArray(const Lattice &lat, real beta, int SpinVol, const string &name)
    : WeightBase(lat, beta, SpinVol, name)
{
    ...
    for (int i = 0; i < lat.Dimension; i++)
        SpaceTimeShape[i] = lat.Size[i];
    SpaceTimeShape[lat.Dimension] = MAX_BIN;
    //Lx*Ly*Lz*Lt
...
unsigned int *WeightArray::Shape()
{
    //the shape of Weight array start from _Shape[SP] to _Shape[TAU]
    return _Shape + SP;
}

unsigned int WeightArray::Size()
{
    return Weight.Size();
}

void WeightArray::Reset(real beta)
{
    _Beta = beta;
    //TODO: please implement how to reset the weight here
}

void WeightArray::SetTest()
{
    for (unsigned int i = 0; i < Weight.Size(); i++) {
        Weight(i) = Complex(1.0, 0.0);
    }
}

void WeightArray::Save(const std::string &FileName, std::string Mode)
{
    cnpy::npz_save(FileName, _Name + ".Weight", Weight(), Shape(), 4, Mode);
    cnpy::npz_save(FileName, _Name + ".DeltaTWeight", DeltaTWeight(), Shape(), 3, Mode);
}

bool WeightArray::Load(const std::string &FileName)
{
    ...
    if (weight.data == nullptr)
        ABORT("Can't find " << _Name << " _Accu in .npz data file!");
    //assignment here will copy data in weight.data into _Weight
    Weight = reinterpret_cast<Complex *>(weight.data);
...
*   which is required by fft on spatial dimensions
*/
bool WeightArray::_CheckVec2Index()
{
    Vec<int> v;
...
WeightEstimator::WeightEstimator(const Lattice &lat, real beta, int SpinVol,
                                 const string &name, int order, real Norm)
    : WeightBase(lat, beta, SpinVol, name)
{
    _Order = order;
    _Shape[ORDER] = _Order;
...
unsigned int *WeightEstimator::Shape()
{
    //the shape of Weight array start from _Shape[ORDER] to _Shape[TAU]
    return _Shape;
}

Estimate<Complex> WeightEstimator::WeightWithError(int order)
{
...
void WeightEstimator::GetWeightArray(WeightArray &WeightArray_, int UpToOrder)
{
    ...
    WeightArray_.Weight = _WeightAccu[order - 1];
    //add order>1 on _Weight
    for (order = 2; order <= UpToOrder; order++)
        WeightArray_.Weight += _WeightAccu[order - 1];
    WeightArray_.Weight *= NormFactor;
}
...
void WeightEstimator::SqueezeStatistics(real factor)
{
    ...
    _NormAccu /= factor;
    _WeightAccu /= Complex(factor, 0.0);
    _Average.SqueezeStatistics(factor);
}

/**********************   Weight IO ****************************************/
void WeightEstimator::Save(const std::string &FileName, std::string Mode)
{
    unsigned int shape[1] = {1};
    cnpy::npz_save(FileName, _Name + ".Norm", &_NormAccu, shape, 1, Mode);
